...
def scene_bdvd(csStore):
    ...
    def onClick(event):
        ...
        bdvd_pause = True
        switch_scene(csStore,csStore["last_scene"]["name"])
    ...
    # Start loop
    if fxEnabled == True:
        csStore["soundSys"].laySound(f"{csStore['GAME_PARENTPATH']}\\assets\\scenes\\bdvd\\background.wav",loop=True)
    # Scene Loop Scheduler Fix:
    csStore["gameLoading_doLoop"] = False
    # Define variables
    bdvd_pos = [0,0]
    bdvd_logo_images = []
    bdvd_logo_index = 0
    ...
    # Define menu loop
    def internal_loop():
        nonlocal bdvd_pause,canvas,bdvd_logo_images,bdvd_logo_index,bdvd_logo,hits
        # Update the canvas
        canvas.update()
        # Move loop
